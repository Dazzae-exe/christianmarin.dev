...
export const ProjectsList = () => {
    ...
    if (isLoading) {
        return <div className="mt-16 max-w-2xl mx-auto">Loading...</div>;
    }
        ...
        <div className="mt-16">
            ...
            <div className="max-w-2xl my-6 md:px-0 px-6 mx-auto space-y-6">
                {projects.map((project) => (
                    <ContentCard key={project.id} title={project.title} href={`/projects/${project.id}`} description={project.description || ''} tags={[]} />
                ))}
            </div>
        </div>
    );
